...
export default {
  ...
  methods: {
    ...
    upload () {
      let resolve, reject
      let promise = new Promise(function (r0, r1) {
        resolve = r0
        reject = r1
      })
      if (!this.changed) {
      ...
      http(Config.api, {bucket: this.bucket}, (result) => {
          ...
          http(Config.qiniuApi, params, (content) => {
              ...
              http(uploadApi[0], formData, () => {
                ...
                reject([this.name, '上传失败'])
              }, {credentials: 'omit', ignoreDefaultParams: true, preProcess: false, ignoreBlock: true})
            }, () => {
              reject([this.name, qnQueryError])
            }, {credentials: 'omit', ignoreDefaultParams: true, method: 'get', preProcess: false, ignoreBlock: true})
        },
        (codes, msgs) => {
          reject([this.name, msgs||'token获取失败'])
        }, {ignoreBlock: true})
      return promise
    },
    getHosts (hosts) {                                                
